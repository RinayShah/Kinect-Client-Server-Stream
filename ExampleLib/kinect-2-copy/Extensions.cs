...
namespace KinectStreams
{
    public static class Extensions
    {
        ...
        {
            if (SkeletonAsString(body, time, out var position))
            return;
            File.AppendAllText(filePath, position);
        }
       public static bool SkeletonAsString(this Body body, string time, out string position)
        {
            if (body == null)
            {
            ...
            {
                sb.Append($" {bodyJoint.Value.Position.X}, {bodyJoint.Value.Position.Y}, {bodyJoint.Value.Position.Z},");
            }
